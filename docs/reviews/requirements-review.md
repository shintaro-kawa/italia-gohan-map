...
## 追加レビュー: Phase 5 スコープ（F-13 / F-14 / F-15、2026-05-18）

### 対象
- F-13 の詳細仕様化（URL クエリ同期含む）
- F-14 画像表示（モーダルのみ、カード非表示で軽量維持）
- F-15 Nominatim ジオコーディング（rate limit + User-Agent 遵守）

### 観点と判定

- **D-005 部分撤回の正当性**: ✓ MVP リリース後、運用で「写真なし」がボトルネック化。詳細モーダルのみ表示で軽量原則は維持
- **Nominatim ToS 遵守**: ✓ 1 req/sec wait + User-Agent 付与が明記、commercial use ではないため OK
- **既存機能との衝突**: なし。F-13 は既存フィルタチェーンに追加、F-14/15 は単独機能
- **要件文言の曖昧さ**: なし

### 指摘
なし。Phase 5 4 タスク（残骸除去・F-13・F-14・F-15）+ CI を **APPROVED**。
